...
- **玩家戰機** — R-82 Ironwing（紅棕色重裝戰鬥機，PNG 圖片），配備雙翼端砲管、天線塔、大型引擎噴口
- **飛機外觀 Profile** — 模組化的戰機外觀定義系統，支援自訂繪製、尾焰粒子效果
- **道具系統** — 三種道具隨機掉落：三連發（綠）、護盾（藍，抵擋下一次傷害）、炸彈（橙，按 Space 使用，最多持有 3 顆，清除畫面所有普通敵人與敵方子彈）
- **特效** — 碰撞時觸發鏡頭震動與閃爍特效
...
│   └── assets/
│       ├── image/          # 圖片資源
│       │   └── player.png  # 玩家戰機圖片（R-82 Ironwing）
│       └── audio/          # 音效資源
│           ├── bgm_menu.wav
│           └── bgm_game.wav
└── src/
